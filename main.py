...
# initial_table = np.random.default_rng().choice(9, size=9, replace=False).reshape(3, 3)
# initial_pos0 = tuple(*np.argwhere(initial_table == 0))
...
if args['alg'] == 1:
    ...
    solution_node = solver.solveDFS(root_node)
else:
    os.system('exit')
